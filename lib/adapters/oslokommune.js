...
module.exports = class OsloKommuneAdapter extends BaseAdapter {
  ...
  // The provider's entries for the address, empty if it has none. It filters on street code,
  // number and letter itself, so a non-empty result means it has data for the address.
  async _fetchResults(addressData) {
  ...
  async coversAddress(addressData) {
    const results = await this._fetchResults(addressData);
  ...
  _nearestDates(disposalArray, today = new Date()) {
  ...
  async getFractionDates(addressData) {
    ...
    const result = results.find(matchesAddress)
      || results.find(matchesStreetNumber)
      || results[0];
    ...
    const pickups = this._nearestDates(points.flatMap((point) => point.Tjenester || []));
